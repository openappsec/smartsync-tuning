...
const (
	...
	kafkaConsumerLogEventID = "ee8824ec-cb51-44f3-b8ff-a1a192cee7e3"

	// MessageHeaderKeyTopic present the message header topic key
	MessageHeaderKeyTopic = "__topic"
)
...
func (c *Consumer) Initialize(kafkaConfiguration ConsumerConfig) error {
	...
	if kafkaConfiguration.DriverLogsEnabled {
		readerConfig.Logger = newKafkaLogger(log.DebugLevel, kafkaConsumerLogEventID)
	}
...
func (c *Consumer) Consume(ctx context.Context) (body []byte, headers map[string][]byte, msg kafkago.Message, err error) {
	...
	headers[MessageHeaderKeyTopic] = []byte(msg.Topic)
