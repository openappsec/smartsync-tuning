...
const (
	kafkaProducerLogEventID = "75f0207a-926b-4828-8220-9faf819bb8b7"
)
...
func (p *Producer) Initialize(kafkaConfiguration ProducerConfig) error {
	...
	if kafkaConfiguration.DriverLogsEnabled {
		writerConfig.Logger = newKafkaLogger(log.DebugLevel, kafkaProducerLogEventID)
	}
...
func (p *Producer) HealthCheck(ctx context.Context) (string, error) {
	...
	return checkName, errors.Errorf("Could not reach all brokers (%d), failing health check (errors: %+v)",
		len(p.brokers), errs)
}
