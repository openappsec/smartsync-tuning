...
func (cm *ConsumerManager) Add(ctx context.Context, kafkaConfiguration kafka.ConsumerConfig, f kafka.HandleFunc) error {
	...
	if _, ok := cm.conAndFunc[consumerTopic(kafkaConfiguration.Topic)]; ok {
		return errors.Wrapf(err, "Could not initialize topic for config: %+v, topic listener already exist", kafkaConfiguration)
	}

	cm.conAndFunc[consumerTopic(kafkaConfiguration.Topic)] = consumerAndFunc{
		consumer: c,
		function: f,
	}
			...
			for {
				...
				if caf.consumer.IsCommitAfterHandlingMsg() {
					if err := caf.consumer.CommitMessage(ctx, msg); err != nil {
						log.WithContext(ctx).Errorf("Failed to commit message (%+v): %s", kafka.FromKafkaMessage(msg), err)
					}
				}
			}
		}(caf)
...
func (cm *ConsumerManager) initRetryConsumers(retryPolicy *kafka.RetryPolicy, kafkaConfiguration kafka.ConsumerConfig, f kafka.HandleFunc) error {
	for retry := 0; retry < retryPolicy.MaxAttempts-1; retry++ {
		...
		cm.conAndFunc[consumerTopic(kafkaRetryConfig.Topic)] = consumerAndFunc{
			consumer: cRetry,
			function: f,
		}
	}
...
// TopicHasMoreAttempts check if consumer has more attempts left
func (cm *ConsumerManager) TopicHasMoreAttempts(ctx context.Context, topic string) (bool, error) {
	if _, ok := cm.conAndFunc[consumerTopic(topic)]; !ok {
		return false, errors.Errorf("topic %s doesn't exist", topic)
	}

	retryPolicy := cm.conAndFunc[consumerTopic(topic)].consumer.GetRetryPolicy()
	if retryPolicy == nil {
		return false, nil
	}

	return hasMoreAttempts(retryPolicy.CurrentAttempt, retryPolicy.MaxAttempts), nil
}
