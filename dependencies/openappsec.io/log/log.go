...
// WithEventID instantiates a log with event ID and calls log.WithFields on it
func WithEventID(eventID string) *Log {
	l := newLog()
	l.entry = l.entry.WithFields(logrus.Fields{EventID: eventID})
	return l
}
...
func (f *Format) handleKnownStringField(k string, vString string) {
	...
	case CallingService:
		f.EventSource.CallingService = vString
